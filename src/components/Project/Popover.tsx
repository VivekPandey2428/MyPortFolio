...
const bgColor: any = ["#808000", "#110322", "#151515", "#b9c7ab"];
...
const Popover = ({ open, currentItem }: any) => {
  ...
  useEffect(() => {
    ...
    const labelCursorX = gsap.quickTo(labelRef.current, "left", {
      duration: 0.4,
      ease: "power3",
    });
    const labelCursorY = gsap.quickTo(labelRef.current, "top", {
      duration: 0.4,
      ease: "power3",
    });
  ...
  return (
    <>
      <motion.div
        ...
        className="popover pointer-events-none"
      >
        <motion.div
          style={{ top: currentItem.idx * -100 + "%" }}
          className="popover-slider"
        >
          {popoverContent?.map((val: any, idx: number) => {
            return (
              <motion.div
                className="popover-img"
                style={{ background: bgColor[idx] }}
              >
                <img src={val} height={0} width={280} alt="project" />
              </motion.div>
            );
          })}
        </motion.div>
      ...
      <motion.div
        ...
        className={classNames(
          "absolute z-30 -translate-x-2/4 cursor-pointer -translate-y-2/4 h-16 w-16 bg-white rounded-full flex justify-center items-center pointer-events-none",
          { hidden: !open }
        )}
      ></motion.div>
      <motion.div
        ref={labelRef}
        className={classNames(
          "text-sm bg-transparent absolute z-30 -translate-x-2/4 cursor-pointer -translate-y-2/4 h-16 w-16 rounded-full flex justify-center items-center pointer-events-none",
          { hidden: !open }
        )}
      >
        View
